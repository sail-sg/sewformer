...
def qlCreatePattern(curves_group):
    ...
    patterns = list(set(objects_after) - set(objects_before))
    return patterns
...
def qlCreateAttachConstraint(points, target):
    ...
    objects_after = cmds.ls(assemblies=True)
    constraints = list(set(objects_after) - set(objects_before))
    constraints = [cmds.rename(cl, target + "_" + cl) for cl in constraints]
    return constraints
